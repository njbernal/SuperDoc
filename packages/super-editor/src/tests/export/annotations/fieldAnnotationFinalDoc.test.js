...
describe('AnnotationNodeExporter for final doc', async () => {
  ...
  it('export image annotation correctly', async() => {
    ...
    expect(imageNode.elements[0].elements[4].elements[0].elements[0].elements[1].elements[0].attributes['r:embed']).toBe(params.relationships[0].attributes.Id);
  });

  it('export signature annotation correctly', async() => {
    const imageNode = body.elements[4].elements[1].elements[0];
    expect(imageNode.elements[0].elements[4].elements[0].elements[0].elements[1].elements[0].attributes['r:embed']).toBe(params.relationships[1].attributes.Id);
  });
