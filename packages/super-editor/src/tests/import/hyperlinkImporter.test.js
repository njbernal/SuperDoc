...
describe('HyperlinkNodeImporter', () => {
  it('parses w:hyperlink with styles', async() => {
    ...
    const { nodes } = handleHyperlinkNode({ nodes: [content[1].elements[2]], docx, nodeListHandler: defaultNodeListHandler() });
    const { marks } = nodes[0];
    ...
    expect(marks[1].attrs.href).toBe('https://stackoverflow.com/questions/66669593/how-to-attach-image-at-first-page-in-docx-file-nodejs');
