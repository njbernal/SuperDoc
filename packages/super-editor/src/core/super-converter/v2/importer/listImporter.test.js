...
describe('table live xml test', () => {
  ...
  it('parses multi nested list xml', () => {
    ...
    expect(result.nodes[0].content[0].content[1].content[0].content[0].content[0].type).toBe(
      'text',
    );
    expect(result.nodes[0].content[0].content[1].content[0].content[0].content[0].text).toBe(
      'L2: B',
    );
  });
});
