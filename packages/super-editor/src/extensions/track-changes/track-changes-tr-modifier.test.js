...
describe('Track Changes TR Modifier', () => {
  ...
  describe('handleMarks', () => {
    it('properly handles a mark add to a non marked node', () => {
      ...
      const trackMarksMark = doc3Json.content[0].content[0].marks.find(
        (mark) => mark.type === TrackMarksMarkName,
      );
    ...
    it('properly handles before/after marks on a node at mark add', () => {
      ...
      const boldMark = doc3Json.content[0].content[0].marks.find((mark) => mark.type === 'bold');
      expect(boldMark).toBeTruthy();
      const italicMark = doc3Json.content[0].content[0].marks.find(
        (mark) => mark.type === 'italic',
      );
      expect(italicMark).toBeTruthy();
      const textStyleMark = doc3Json.content[0].content[0].marks.find(
    ...
    it('properly handles before/after marks on a node at mark attribute change', () => {
      ...
      tr.insert(0, node);
      tr.addMark(1, 5, state.schema.marks['bold'].create());
      const textStyleMark = state.schema.marks['textStyle'].create({
        fontSize: '14pt',
        color: '#FF004D',
        fontFamily: null,
      });
      ...
      const boldMark = doc3Json.content[0].content[0].marks.find((mark) => mark.type === 'bold');
      expect(boldMark).toBeTruthy();
      const textStyleMark2 = doc3Json.content[0].content[0].marks.find(
        (mark) => mark.type === 'textStyle',
      );
